...
```{r anovas}
foreach(i=n) %do% anova(lm3_models[[i]]$finalModel)
foreach(i=n) %do% car::Anova(lm3_models[[i]]$finalModel)
```
...
foreach(i=n) %do% mse(i, test=FALSE, lm3_models)
print('MSE, RMSE and MAE for test')
foreach(i=n) %do% mse(i, test=TRUE, lm3_models)
