...
result <- data.frame( maker = NA, model= NA, mileage = NA,   manufacture_year = NA, engine_displacement= NA, engine_power   = NA,  body_type    = NA,transmission  = NA,  door_count    = NA, seat_count    = NA, fuel_type = NA, date_created  = NA, price_eur= NA)
...
train$stk_year <- relevel( as.factor(train$stk_year), ref = '2018') 
...
lm3_models <- foreach(i=n) %do% lm_for_stratas(x = c('mileage', 'manufacture_year', 'engine_displacement', 'engine_power', 'body_type', 'transmission', 'door_count'), y = 'price_eur', i)

# Should I group levels in manufacture_year?
```
